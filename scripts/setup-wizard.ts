...
async function main() {
  ...
  if (existsSync(identityPath) && existsSync(envPath)) {
    console.log("");
    console.log(`  ${theme.warning}Setup already complete.${theme.reset}`);
    console.log(`  ${theme.muted}To reconfigure, delete .env and data/lumiverse.identity.${theme.reset}`);
    console.log("");
    process.exit(0);
  }
  ...
  printStepHeader(4, 4, "Generating Identity", "Creating your encryption identity and configuration.");
  ...
  const authSecret = bytesToHex(crypto.getRandomValues(new Uint8Array(32)));
  ...
  printSummary(
    ...
    [
      "Keep data/lumiverse.identity safe.",
      "It is your encryption key. If lost, encrypted secrets cannot be recovered.",
    ]
  );
