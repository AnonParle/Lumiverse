...
export function createPersona(userId: string, input: CreatePersonaInput): Persona {
  ...
  getDb()
    .query(
      `INSERT INTO personas (id, user_id, name, description, is_default, attached_world_book_id, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(id, userId, input.name, input.description || "", input.is_default ? 1 : 0, input.attached_world_book_id || null, JSON.stringify(input.metadata || {}), now, now);
...
export function duplicatePersona(userId: string, id: string): Persona | null {
  ...
  getDb()
    .query(
      `INSERT INTO personas (id, user_id, name, description, avatar_path, image_id, is_default, attached_world_book_id, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?)`
    )
    .run(
      newId,
